...
def distribute_follow_state_update(rel, payload: dict) -> None:
    """
    Send the current follow relationship state back to the remote follower's inbox.
    Used when approving a remote follower so their node can update the existing
    outgoing follow request to APPROVED.
    """
    remote_author = rel.follower
    remote_node = get_remote_node_for_author(remote_author)
    inbox_url = remote_inbox_url_for_author(remote_author)
